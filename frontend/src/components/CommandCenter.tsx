...
export const CommandCenter: React.FC<CommandCenterProps> = ({
  ...
  const getRiskBadge = (lvl: string) => {
    switch (lvl) {
      case 'CRITICAL': return { bg: 'bg-red-600', text: 'text-red-700', symbol: '🔴', label: tr('CRITICAL') };
      case 'HIGH': return { bg: 'bg-orange-500', text: 'text-orange-700', symbol: '🟠', label: tr('HIGH') };
      case 'MODERATE': return { bg: 'bg-amber-500', text: 'text-amber-700', symbol: '🟡', label: tr('MODERATE') };
      default: return { bg: 'bg-emerald-600', text: 'text-emerald-700', symbol: '🟢', label: tr('LOW') };
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 font-bold">{tr('Target Inspection Ward:')}</span>
              <select
            ...
            {activeFlashWarning && activeFlashWarning.status !== 'NONE' && (
              <div className="flex items-center gap-2 text-xs font-black text-red-700 bg-red-50 border border-red-200 px-3 py-1.5 rounded-xl">
                ...
                <span>{tr('ACTIVE FORECAST:')} {tr(activeFlashWarning.statusLabel)} ({tr(activeFlashWarning.estimatedWindow)})</span>
              </div>
            )}
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div>
                <div className="text-xs font-black text-slate-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  ...
                  <span>{tr('Real-Time Sensor Telemetry Status')}</span>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">{tr('Rainfall Rate')}</span>
                    ...
                    <span className={`text-[10px] font-bold block mt-0.5 ${
                      ...
                      {selectedLocation.rainfall > 60 ? tr('CRITICAL INFLOW') : selectedLocation.rainfall > 25 ? tr('MODERATE RAIN') : tr('NORMAL')}
                    </span>
                  </div>

                  <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">{tr('Drain Efficiency')}</span>
                    ...
                    <span className={`text-[10px] font-bold block mt-0.5 ${
                      ...
                      {selectedLocation.drainage_condition < 35 ? tr('CHOKED DRAIN') : tr('FLOWING NORMALLY')}
                    </span>
                  </div>

                  <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">{tr('River Gauge')}</span>
                    <span className="text-lg font-black text-slate-900 font-mono">
                      {selectedLocation.sensor_water_level || 4.2} m
                    </span>
                    <span className="text-[10px] text-amber-600 font-bold block mt-0.5">
                      {tr('WARNING THRESHOLD')}
                    </span>
                  </div>

                  <div className="bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
                    <span className="text-[10px] uppercase font-bold text-slate-400 block">{tr('Citizen Logs')}</span>
                    <span className="text-lg font-black text-red-600 font-mono">{reportsCount} {tr('REPORTS')}</span>
                    <span className="text-[10px] text-slate-500 font-bold block mt-0.5">
                      {tr('Verified Local Hazard')}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="bg-white rounded-3xl border-2 border-slate-200 p-5 sm:p-6 shadow-xs space-y-4">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">
                      {tr('AREA UNDER REVIEW')}
                    </span>
                    <h3 className="text-lg font-black text-slate-900 font-mono">
                      {tr(selectedLocation.name.toUpperCase())}
                    </h3>
                  </div>
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
                  <span className="text-[11px] font-black uppercase tracking-wider text-slate-700 block">
                    {tr('WHY IS THE RISK')} {tr(activeRisk)}?
                  </span>
                  <div className="space-y-1.5 text-xs text-slate-800">
                    <div className="flex items-center gap-2">
                      <span>🌧️</span>
                      <span className="font-semibold">{tr('Rainfall:')}</span>
                      <strong className="text-slate-900">
                        {selectedLocation.rainfall > 60 ? tr('High') : selectedLocation.rainfall > 25 ? tr('Moderate') : tr('Low')} ({selectedLocation.rainfall} mm/h)
                      </strong>
                    </div>
                    <div className="flex items-center gap-2">
                      <span>💧</span>
                      <span className="font-semibold">{tr('Soil moisture:')}</span>
                      <strong className="text-slate-900">
                        {selectedLocation.soil_moisture > 75 ? tr('High (Saturated)') : tr('Normal')}
                      </strong>
                    </div>
                    <div className="flex items-center gap-2">
                      <span>🚧</span>
                      <span className="font-semibold">{tr('Drainage:')}</span>
                      <strong className={selectedLocation.drainage_condition < 40 ? 'text-red-600' : 'text-slate-900'}>
                        {selectedLocation.drainage_condition < 40 ? tr('Poor / Choked') : tr('Good')}
                      </strong>
                    </div>
                  </div>
                  <p className="text-[11px] text-slate-500 font-medium pt-1 border-t border-slate-200 mt-2">
                    {tr('These conditions are increasing local flood risk.')}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-red-700">
                    {reportsCount} {tr('citizen reports logged in this sector')}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-900">
                    {selectedLocation.id === 'ward-12' 
                      ? `${tr('Main Market Road')}, Station Road, Culvert #4 Crossing`
                      : `${tr('Valley Riverside Road')}, Lower Ghat Terraces`}
                  </p>
                </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <span className={completedActions.action_1 ? 'line-through text-slate-500' : 'font-semibold'}>
                        {tr('Inspect drainage channel and deploy de-silting crew')}
                      </span>
                    </div>
                    ...
                    >
                      ...
                      <span className={completedActions.action_2 ? 'line-through text-slate-500' : 'font-semibold'}>
                        {tr('Close affected road if water rises further; position police barricades')}
                      </span>
                    </div>
                    ...
                    >
                      ...
                      <span className={completedActions.action_3 ? 'line-through text-slate-500' : 'font-semibold'}>
                        {tr('Broadcast municipal SMS flood warning to residents in Ward 12')}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-black uppercase tracking-wider text-slate-900">
                      {tr('Citizen Hazard Reports')}
                    </h4>
                  </div>
                  <span className="text-[10px] font-bold text-slate-400 font-mono">
                    {citizenReports.length} {tr('Total Logs')}
                  </span>
                </div>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {citizenReports.slice(0, 3).map((rep) => (
                    <div key={rep.id} className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 text-xs flex items-center justify-between gap-2">
                      <div>
                        <span className="font-bold text-slate-900 block">{tr(rep.location_name)}</span>
                        <span className="text-slate-600 text-[11px]">{tr(rep.description)}</span>
                      </div>
              ...
              <div className="bg-slate-50 rounded-3xl border border-slate-200 overflow-hidden">
                ...
                >
                  <span>{tr('TECHNICAL DETAILS & MODEL DIAGNOSTICS')}</span>
